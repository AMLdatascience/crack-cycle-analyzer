...
plt.plot(img_cycle['cycle'].values, od_preds['data']['instance_score'], 'r--o')
...
## 결과 저장 : 3. 스코어 데이터 저장
out_data = pd.concat([img_cycle, pd.DataFrame(od_preds['data']['instance_score']).reset_index(drop=True)], axis=1)
out_data.rename(columns={0:"Crack Score"}, inplace = True)
